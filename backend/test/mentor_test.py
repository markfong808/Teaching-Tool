...
class MentorTestCase(unittest.TestCase):
    ...
    @patch('api.mentor.get_jwt_identity')
    @patch('api.mentor.Availability.query')
    @patch('api.mentor.db.session')
    def test_add_mentor_availability(self, mock_db_session, mock_availability_query, mock_jwt_identity):
        # Set up mocks
        ...
        response = self.client.post('/mentor/add-availability', data=json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, 201)

    # Tests for other endpoints and functions...
    @patch('api.mentor.Appointment.query')
    @patch('api.mentor.db.session')
    @patch('api.mentor.send_confirmation_email')
    ...
    def test_set_auto_approve(self, mock_db_session, mock_user_query, mock_jwt_identity):
        ...
        mock_jwt_identity.return_value = 1
        mock_mentor = MagicMock(spec=User)
        mock_user_query.filter_by.return_value.first.return_value = mock_mentor
        mock_db_session.commit = MagicMock()

        # Mock data for the request
        data = {'auto_approve': True}
        response = self.client.post('/mentor/settings/auto-approve', data=json.dumps(data), content_type='application/json')

        self.assertEqual(response.status_code, 200)
        mock_user_query.filter_by.assert_called_with(id=1, account_type='mentor')
        self.assertTrue(mock_mentor.auto_approve_appointments)
        mock_db_session.commit.assert_called()
    ...
    @patch('api.mentor.db.session')
    @patch('api.mentor.send_confirmation_email')
    def test_update_mentor_appointment_status(self, mock_send_email, mock_db_session, mock_appointment_query, mock_jwt_identity):
        # Mock get_jwt_identity, Appointment query, db session, and send_confirmation_email
        mock_jwt_identity.return_value = 1
        mock_appointment = MagicMock(spec=Appointment, id=1, mentor_id=1, status='pending')
        ...
        response = self.client.post('/mentor/appointments/pending', data=json.dumps(data), content_type='application/json')
        
        self.assertEqual(response.status_code, 201)
        mock_appointment_query.filter_by.assert_called_with(id=1, mentor_id=1)
    ...
    @patch('api.mentor.User.query')
    @patch('api.mentor.Availability.query')
    def test_get_mentor_availability(self, mock_availability_query, mock_user_query, mock_jwt_identity):
        # Mock get_jwt_identity, User query, and Availability query
        mock_jwt_identity.return_value = 1
        mock_mentor = MagicMock(spec=User, id=1, account_type='mentor')
        mock_user_query.filter_by.return_value.first.return_value = mock_mentor

        mock_availability = MagicMock(spec=Availability)
        mock_availability.id = 1
        mock_availability.type = 'Mentoring Session'
        ...
        response = self.client.get('/mentor/availability')
    ...
    @patch('api.mentor.Availability.query')
    @patch('api.mentor.db.session')
    def test_delete_mentor_availability(self, mock_db_session, mock_availability_query, mock_user_query, mock_jwt_identity):
        ...
        availability_id = '1'
        response = self.client.delete(f'/mentor/delete-availability/{availability_id}')
    ...
    def test_cancel_appointment(self, mock_db_session, mock_user_query, mock_appointment_query, mock_jwt_identity):
        ...
        mock_jwt_identity.return_value = 1
        mock_mentor = MagicMock(spec=User, id=1)
        mock_user_query.get.return_value = mock_mentor

        future_date = datetime.now() + timedelta(days=1)
        mock_appointment = MagicMock(spec=Appointment, id=1, mentor_id=1, appointment_date=future_date.strftime('%Y-%m-%d'), start_time='10:00', status='reserved')
        mock_appointment_query.get.return_value = mock_appointment
        mock_db_session.commit = MagicMock()

        appointment_id = '1'
        response = self.client.post(f'/mentor/appointments/cancel/{appointment_id}')
