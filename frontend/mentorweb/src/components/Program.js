      ...
      <div className="w-3/4 p-5 m-auto">
        <div className="flex items-center">
          ...
          >
            ...
            <h1><strong>Select Program type:</strong></h1>
                <select
                    className='border border-light-gray rounded ml-2'
                    //id="course-dropdown"
                ...
                <option key={-1} value="">Select...</option>
                    {allCourseData.map((course) => (
                    <option key={course.id} value={course.id}>{course.class_name}</option> // Trent Implements 
                    ))}
                </select>
                <button className="font-bold border border-light-gray rounded-md shadow-md text-sm px-1 py-1 ml-4">Create Program Type </button>
                <input className='border border-light-gray ml-2'/>
            </div>
        </div>
      </div>
