...
export default function Users() {
    ...
    const handleAccountTypeChange = (user_id, new_account_type, userName) => {
        if (!new_account_type ||
            !window.confirm(`Are you sure you want to change ${userName}'s account type to ${new_account_type}?`)) {
            return;
        }
        ...
        fetch("/admin/change-account-type", {
            ...
            .then((response) => {
                if (response.ok) {
                    ...
                    setUsers((prevUsers) =>
                        prevUsers.map((user) =>
                            user.id === user_id ? { ...user, account_type: new_account_type } : user
                        )
                    );
                } else {
    ...
    const handleAccountStatusChange = (user_id, new_account_status, userName) => {
        if (!new_account_status ||
            !window.confirm(`Are you sure you want to change ${userName}'s account status to ${new_account_status}?`)) {
            return;
        }
        fetch("/admin/change-account-status", {
            ...
            .then((response) => {
                if (response.ok) {
                    ...
                    setUsers((prevUsers) =>
                        prevUsers.map((user) =>
                            user.id === user_id ? { ...user, status: new_account_status } : user
                        )
                    );
                } else {
        ...
        <div className="flex flex-col w-2/3 m-auto">
            {selectedUser ? (
                ...
                <div className="">
                    ...
                    <select className="border border-light-gray" onChange={handleUserTypeChange}>
                    ...
                    {selectedUserType && (
                        <div>
                            <input className="border border-light-gray w-1/3 my-5"
                                type="text"
                                placeholder="Filter"
                                    ...
                                    {groupUsersByAccountType[selectedUserType]
                                        ...
                                        .map((user) => (
                                            <tr className="border-b" key={user.id}>
                                                ...
                                                <td className="border-r text-blue underline cursor-pointer" onClick={() => handleUserNameClick(user)}>{user.name}</td>
                                                <td className="border-r">{user.email}</td>
                                                <td className="border-r w-[125px]">
                                                    <div>
                                                        <select onChange={(e) => handleAccountTypeChange(user.id, e.target.value, user.name)}>
                                                            <option value="">{capitalizeFirstLetter(user.account_type)}</option>
                                                ...
                                                <td className="border-r w-[125px]">
                                                    <div>
                                                        <select onChange={(e) => handleAccountStatusChange(user.id, e.target.value, user.name)}>
                                                            <option value="">{capitalizeFirstLetter(user.status)}</option>
