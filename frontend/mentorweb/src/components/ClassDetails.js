...
export default function ClassDetails() {
  ...
  const handleInputChange = async (e) => {
    ...
    const selectedCourse = allCourseData.find(
      (course) => course.id === selectedClassData.id
    );

    // Update showButtons state
    setShowSaveCancelButtons((prevButtons) => ({
      ...prevButtons,
      [name]: value === selectedCourse[name],
    }));
  };
    ...
    if (allTimesData.length > 0) {
      if (tempValue.length === 0) {
        setAllTimesData((prevTimesData) => {
          ...
          const updatedTimesData = prevTimesData.filter(
            (entry) => !(entry.day === tempDay && entry.type === tempType)
          );
      ...
      // to add a time block
        ...
        setAllTimesData((prevTimesData) => {
          ...
          const existingEntryIndex = prevTimesData.findIndex(
            (entry) => entry.day === tempDay
          );
      ...
      <div className="flex flex-col w-3/4 p-5 m-auto border border-light-gray rounded-md shadow-md">
        ...
        <h2 className="pb-10 text-center font-bold text-2xl">Class Details</h2>
          <div className="flex flex-col">
            ...
            <div>
              <MeetingLocation
                isClassLocation={true}
                param={{
                  functionPassed: handleInputChange,
                  loadPageFunction: setClassLocRec,
                  changesMade: setChangesMade,
                }}
                data={{
                  class_location: selectedClassData.class_location,
                  class_recordings_link:
                    selectedClassData.class_recordings_link,
                  class_link: selectedClassData.class_link,
                }}
                loadPage={loadClassLocRec}
                changes={changesMade}
              />
            </div>
